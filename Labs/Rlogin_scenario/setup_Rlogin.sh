...
echo "=== 6. Test de Rlogin ==="
echo "Configuration du daemon rlogind et de la faille .rhosts..."

#on configure inetd pour ecouter le port rlogin(513)

docker exec server useradd  -m -s  /bin/bash etudiant || true
docker exec server bash -c "echo 'login stream tcp nowait root /usr/sbin/in.rlogind' > /etc/inetd.conf"
docker exec server pkill inetd || true
docker exec server /usr/sbin/inetd

docker exec server bash -c "echo '+ +' > /home/etudiant/.rhosts"

docker exec server chmod 600 /home/etudiant/.rhosts
docker exec server chown etudiant:etudiant /home/etudiant/.rhosts

#2.Lancement de la sonde 

docker exec sonde pkill tcpdump || true
docker exec -d sonde tcpdump -i veth-snd-cont -n -U -w /data/capture_rlogin.pcap
sleep 2

#3.Le transfert de commandes automatises
#Grace au fichier .rhost le serveur ne demande pas de mot de passe 
docker exec client bash -c "(sleep 1; echo 'ls -la'; sleep 1; echo 'whoami'; sleep 1; echo 'exit') | rlogin 10.1.20.2 -l etudiant"

echo "Arret de la capture ...."
sleep 2

docker exec sonde pkill -2 tcpdump

sleep 1



echo "===TEST Rlogin Termine avec Succes ==="
