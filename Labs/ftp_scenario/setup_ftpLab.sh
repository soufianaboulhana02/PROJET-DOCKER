...
echo "demarrage du service FTP sur le serveur "
docker exec server bash -c "echo 'Contenu de test pour le TME ARES' > /home/etudiant/test_tme.txt"
docker exec server chown etudiant:etudiant /home/etudiant/test_tme.txt

echo "-> Demarrage du service vsftpd..."
docker exec -d server /usr/sbin/vsftpd /etc/vsftpd.conf
sleep 2
echo "Lancement de la capture sur la sonde"
docker exec -d sonde tcpdump -i veth-snd-cont -n -U -w /data/capture_ftp2.pcap
sleep 2
echo "Connexion FTP du client et recupertion du fichier"
docker exec client bash -c "ftp -n 10.1.20.2 <<EOF
user etudiant reseau
passive
get test_tme.txt
quit
EOF"
